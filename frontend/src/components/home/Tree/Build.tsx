...
import { combineClasses, getId, readableMonitorTimestamp } from "../../../util/helpers";
...
const Build: Component<{ id: string }> = (p) => {
  ...
  const version = () => {
    return `v${build().version.major}.${build().version.minor}.${build().version.patch}`
  }
  const lastBuiltAt = () => {
    if (build().last_built_at === "never") {
      return "not built"
    } else {
