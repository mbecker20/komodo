...
static OIDC_CLIENT: OnceLock<Option<CoreClient>> = OnceLock::new();

fn oidc_client() -> Option<&'static CoreClient> {
  OIDC_CLIENT
    .get()
...
async fn init_oidc_client() -> anyhow::Result<()> {
    ...
    .route(
      ...
      get(
        |Path(path): Path<ProviderPath>,
         Query(query): Query<RedirectQuery>| async move {
          // Redirect::to(
          //   &google_oauth_client()
          //     .as_ref()
