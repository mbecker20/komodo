...
use crate::{
  ...
  config::{config_load, into_log_level, Env},
  helpers::{cache::Cache, channel::BroadcastChannel},
...
impl State {
  pub async fn load() -> anyhow::Result<Arc<State>> {
    let env = Env::load()?;
    let mut config = config_load(&env.config_path);
    ...
    let state: Arc<_> = State {
      env,
      db: DbClient::new(&config.mongo).await?,
